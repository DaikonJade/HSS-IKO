...
fname = fname.replace(/$+/, '');        // remove leading $ characters
...
const html = `
  <div style="display:flex;gap:12px;flex-wrap:wrap">
    <img class="detail-img" src="${window.escapeAttr(window.imgUrl(it))}" data-src="${window.escapeAttr(window.imgUrl(it))}" style="max-width:320px;width:100%;border-radius:6px;cursor:zoom-in" alt="">
...
// ensure zoom wiring runs after content is inserted
setTimeout(()=>{ try{ window.enableImageZoom && window.enableImageZoom(); } catch(e){ console.error('enableImageZoom error', e); } }, 0);
};
...
zoom.style.inset = '0';
zoom.style.background = 'rgba(0,0,0,0.85)';
