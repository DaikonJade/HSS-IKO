...
if(!c) return;// clear container entirely (we rebuild)
c.innerHTML = '';
...
// render the checkboxes into each container
...
const c = q(containerId); if(!c) return []; return Array.from(c.querySelectorAll('input[type=checkbox]:checked')).map(i=>i.value);
}
...
// preview now only shows image, title, and type
card.innerHTML =` <img class="thumb" src="${window.escapeAttr ? window.escapeAttr(imgSrc) : imgSrc}" alt="">       <div class="card-body">         <strong>${window.escapeHtml ? window.escapeHtml(it.title||it.jp_title||it.id) : (it.title||it.jp_title||it.id)}</strong>         <div class="meta">${window.escapeHtml ? window.escapeHtml(typeText) : typeText}</div>         <div class="buttons">           <button class="detail-btn">Details</button>           <button class="${wishlist.has(it.id)?'wishlist-btn':''}" data-id="${window.escapeAttr ? window.escapeAttr(it.id) : it.id}">${wishlist.has(it.id)?'In my wishlist':'Add to my wishlist'}</button>         </div>       </div>`;
...
// Make .multi-filter collapsible on small screens (kept for compatibility; populateFilters now builds summary too)
...
// if we already added a summary via earlier helper, don't double-add
...
container._hasSummary = true;// wrap the existing controls + list in a wrapper so we can hide them with CSS
  const wrapper = document.createElement('div');
  wrapper.className = 'multi-filter-list';
...
const detailsLines = [];
if (it.type && it.type.length) detailsLines.push(`<div><strong>类型：</strong>${window.escapeHtml ? window.escapeHtml(it.type.join(', ')) : it.type.join(', ')}</div>`);
if (it.relevant_work && it.relevant_work.length) detailsLines.push(`<div><strong>相关作品：</strong>${window.escapeHtml ? window.escapeHtml(it.relevant_work.join(', ')) : it.relevant_work.join(', ')}</div>`);
if (it.relevant_character && it.relevant_character.length) detailsLines.push(`<div><strong>相关人物：</strong>${window.escapeHtml ? window.escapeHtml(it.relevant_character.join(', ')) : it.relevant_character.join(', ')}</div>`);
if (it.releaser && it.releaser.length) detailsLines.push(`<div><strong>发行商：</strong>${window.escapeHtml ? window.escapeHtml(it.releaser.join(', ')) : it.releaser.join(', ')}</div>`);
if (it.release_date) detailsLines.push(`<div><strong>发行日期：</strong>${window.escapeHtml ? window.escapeHtml(it.release_date) : it.release_date}</div>`);
if (it.release_price) detailsLines.push(`<div><strong>发行价格：</strong>${window.escapeHtml ? window.escapeHtml(it.release_price) : it.release_price}</div>`);
if (it.release_area && it.release_area.length) detailsLines.push(`<div><strong>发行地区：</strong>${window.escapeHtml ? window.escapeHtml(it.release_area.join(', ')) : it.release_area.join(', ')}</div>`);
...
const html = `
  <div style="display:flex;gap:12px;flex-wrap:wrap">
    ...
    <div style="flex:1;min-width:220px">
      ...
      <div style="margin-top:12px"><button onclick="window.toggleWishlist && window.toggleWishlist('${window.escapeAttr ? window.escapeAttr(it.id) : it.id}', this)">${wishlist.has(it.id)?'In my wishlist':'Add to my wishlist'}</button></div>
    </div>
  </div>`;
...
if (closeBtn) closeBtn.addEventListener('click', function () { if (typeof window.closeModal === 'function') window.closeModal(); else { const m = document.getElementById('modal'); if (m) { m.style.display = 'none'; m.setAttribute('aria-hidden', 'true'); } } });
...
async function loadOriginalRows(){ if (window._originalRows) return window.originalRows; const txt = await fetch('data.csv?=' + Date.now()).then(r => r.text()); const parsed = (typeof Papa !== 'undefined') ? Papa.parse(txt.trim(), { header: true, skipEmptyLines: true }).data : []; window._originalRows = parsed; return parsed; }
...
async function exportWishlistCSV(filename){ if (!wishlist || wishlist.size === 0) { alert('Wishlist is empty — nothing to export.'); return; } const usedFilename = filename ||`wishlist-${timestampForFilename()}.csv`; try { const originalRows = await loadOriginalRows(); const idToRows = new Map(); (originalRows || []).forEach((r, i) => { const id = rowIdFromRaw(r, i); if (!idToRows.has(id)) idToRows.set(id, []); idToRows.get(id).push(r); }); const outRows = []; Array.from(wishlist).forEach(id => { const rows = idToRows.get(id); if (rows && rows.length) rows.forEach(r => outRows.push(r)); else outRows.push({ id }); }); let csv; if (typeof Papa !== 'undefined' && typeof Papa.unparse === 'function') csv = Papa.unparse(outRows); else { const keys = Array.from(outRows.reduce((s, r) => { Object.keys(r || {}).forEach(k => s.add(k)); return s; }, new Set())); const lines = [keys.join(',')]; outRows.forEach(r => { lines.push(keys.map(k => { const v = r[k] == null ? '' : String(r[k]).replace(/"/g, '""'); return`"${v}"; }).join(',')); }); csv = lines.join('\n'); } const bom = '\uFEFF'; const blob = new Blob([bom + csv], { type: 'text/csv;charset=utf-8;' }); downloadBlob(blob, usedFilename); } catch (err) { console.error('Export failed', err); alert('Export failed: ' + (err && err.message ? err.message : err)); } }
...
setTimeout(()=>{
const dl = document.getElementById('download-wishlist');
if (dl) dl.addEventListener('click', () => exportWishlistCSV());
}, 50);
}
