...
window.openDetail = async function(id){
  try{
    ...
    // build details block: Type, Relevant Work, Relevant Character, and the release info lines
    const detailsLines = [];
...
async function loadOriginalRows(){ if (window._originalRows) return window._originalRows; const txt = await fetch('data.csv?_=' + Date.now()).then(r => r.text()); const parsed = (typeof Papa !== 'undefined') ? Papa.parse(txt.trim(), { header: true, skipEmptyLines: true }).data : []; window._originalRows = parsed; return parsed; }
