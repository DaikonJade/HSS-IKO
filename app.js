...
fetch(DATA_FILE).then(r => r.text()).then(txt => {
  ...
  // initialize filtered to show everything initially
  ...
  slice.forEach(it => {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="meta">${window.escapeHtml ? window.escapeHtml(typeText) : typeText} • ${window.escapeHtml ? window.escapeHtml(workText) : workText} • ${window.escapeHtml ? window.escapeHtml(charText) : charText}</div>
        <div class="buttons">
...
document.getElementById('next') && (document.getElementById('next').disabled = page >= Math.max(1, Math.ceil(filtered.length / PAGE_SIZE)));
