...
window.renderPage = function(){
  ...
  slice.forEach(it => {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        <strong>${window.escapeHtml(it.title||it.jp_title||it.id)}</strong>
        <div class="meta">$${window.escapeHtml(it.type)} • $${window.escapeHtml(it.relevant_work)} • ${window.escapeHtml(it.relevant_character)}</div>
        <div class="buttons">
          <button class="detail-btn">Details</button>
          <button class="$${wishlist.has(it.id)?'wishlist-btn':''}" data-id="$${it.id}">${wishlist.has(it.id)?'Wanted':'Add'}</button>
        </div>
      </div>`;
...
window.showWishlist = function(){
  ...
  const html = listItems.map(it => `
    <div style="display:flex;gap:10px;margin-bottom:10px">
      <img src="${window.escapeAttr(window.imgUrl(it))}" alt="" style="width:90px;height:90px;object-fit:cover;border-radius:6px">
      <div>
        <strong>${window.escapeHtml(it.title||it.jp_title||it.id)}</strong>
        <div class="small">$${window.escapeHtml(it.type)} • $${window.escapeHtml(it.relevant_work)}</div>
        <div style="margin-top:6px"><button class="ghost" data-remove="${it.id}">Remove</button></div>
      </div>
    </div>
  `).join('');
